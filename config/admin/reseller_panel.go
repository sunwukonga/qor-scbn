...
var ResellerAdmin *admin.Admin

var ResellerActionBar *action_bar.ActionBar

//var Countries = []string{"Singapore", "Philippines", "Indonesia", "Malaysia"}

func init() {
	ResellerAdmin = admin.New(&qor.Config{DB: db.DB.Set(publish2.VisibleMode, publish2.ModeOff).Set(publish2.ScheduleMode, publish2.ModeOff)})
	ResellerAdmin.SetSiteName("SC Reseller")
	ResellerAdmin.SetAuth(auth.AdminAuth{})
	ResellerAdmin.SetAssetFS(bindatafs.AssetFS)

	// Add Asset Manager, for rich editor
	assetManager := ResellerAdmin.AddResource(&media_library.AssetManager{}, &admin.Config{Invisible: true})

	// Add Help
	ResellerHelp := ResellerAdmin.NewResource(&help.QorHelpEntry{})
	ResellerHelp.GetMeta("Body").Config = &admin.RichEditorConfig{AssetManager: assetManager}

	// Add Notification
	// Notifications for Reseller should include new orders. Perhaps abandoned orders.
	/*
		...
		ResellerAdmin.NewResource(Notification)
	*/

	// Add Dashboard
	ResellerAdmin.AddMenu(&admin.Menu{Name: "Dashboard", Link: "/reseller_panel"})
	...
	/*
		...
		category.Meta(&admin.Meta{Name: "Parent", Label: "Parent Category", Type: "select_one", Config: &admin.SelectOneConfig{AllowBlank: true}})
		category.Meta(&admin.Meta{Name: "Children",
			Label: "Subcategories",
			Type:  "select_many",
		})
	...
	*/
	// Add Order
	// TODO:: Limit scope of orders so that reseller only sees orders associated with his or her reseller coupon.
	order := ResellerAdmin.AddResource(&models.Order{}, &admin.Config{Menu: []string{"Order Management"}})
	...
	*/
	...
	ResellerAdmin.AddSearchResource(order)
	//Admin.AddSearchResource(product, user, order)

	// Add ActionBar
	ResellerActionBar = action_bar.New(ResellerAdmin)
	ResellerActionBar.RegisterAction(&action_bar.Action{Name: "Reseller Dashboard", Link: "/reseller_panel"})
